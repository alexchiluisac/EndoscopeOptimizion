function STvisibility = visiblesurface2(wristDesign)
...
%alpha = pi;
cutouts = [];
cutouts.w =  wristDesign(1:n);  %ones(1,n) * 1.20  * 1e-3;
cutouts.u = wristDesign(n+1:2*n);
cutouts.h = wristDesign(2*n+1:3*n);
cutouts.alpha = wristDesign(3*n+1:4*n);
